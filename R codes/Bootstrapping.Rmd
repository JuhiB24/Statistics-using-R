...
```{r}
mess_bill <- read.csv(here("C:/Users/JUHI/OneDrive - IIT Kanpur/Sem 1/Stats/Assignments R/June_mess_bill.csv"))
head(mess_bill)
```
The dataset I selected includes institute’s mess billing data for 150+ residents in Hall 4.
...
First, I calculated the overall mean (population mean).
...
mean(~ReadingScore, data=mess_bill)
```
```{r}
...
samp.mean<-do(10000)*{
mean(~ReadingScore, data=sample(mess_bill, size= 100))
}
...
samp.mean<-do(10000)*{
mean(~ReadingScore, data=sample(mess_bill, size= 5))
}
...
```{r}
samp.mean<-do(10000)*{
mean(~ReadingScore, data=sample(mess_bill, size= 10))
}
